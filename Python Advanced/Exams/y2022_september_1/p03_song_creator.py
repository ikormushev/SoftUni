def add_songs(*args):
    ...
    for (song_name, lyrics) in args:
        ...
        for line in lyrics:
            songs[song_name] += f"{line}\n"

    string_to_print = ""

    for (song_name, lyrics) in songs.items():
        string_to_print += f"- {song_name}\n"
        if lyrics:
            string_to_print += f"{lyrics}"
...
print(add_songs(
    ...
    ("Beat It", []),
    ("Beat It", ["Showin' how funky and strong is your fight", "It doesn't matter who's wrong or right"]),
))
