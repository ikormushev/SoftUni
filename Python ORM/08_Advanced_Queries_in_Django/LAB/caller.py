...
def product_quantity_ordered():
    products = (Product.objects.
                annotate(units_ordered=Sum('orderproduct__quantity')).
                exclude(units_ordered=None).
                order_by('-units_ordered'))

    return "\n".join([f"Quantity ordered of {p.name}: {p.units_ordered}" for p in products])
...
# Task 5
def give_discount():
    Product.objects.filter(Q(price__gt=3.00) & Q(is_available=True)).update(price=F("price") * 0.7)

    return "\n".join([f"{p.name}: {p.price}lv." for p in
