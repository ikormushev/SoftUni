...
class Task(models.Model):
    ...
    def ongoing_high_priority_tasks(cls):
        return cls.objects.filter(Q(priority='High')
    ...
    def recent_completed_tasks(cls, days: int):
        return cls.objects.filter(Q(is_completed=True)
...
class Exercise(models.Model):
    ...
    def get_long_and_hard_exercises(cls):
        return cls.objects.filter(Q(duration_minutes__gt=30)
                                  & Q(difficulty_level__gte=10))

    @classmethod
    def get_short_and_easy_exercises(cls):
        return cls.objects.filter(Q(duration_minutes__lt=15)
                                  & Q(difficulty_level__lt=5))

    @classmethod
    def get_exercises_within_duration(cls, min_duration: int, max_duration: int):
        # return cls.objects.filter(Q(duration_minutes__gte=min_duration)
    ...
    def get_exercises_with_difficulty_and_repetitions(cls, min_difficulty: int, min_repetitions: int):
        return cls.objects.filter(Q(difficulty_level__gte=min_difficulty)
