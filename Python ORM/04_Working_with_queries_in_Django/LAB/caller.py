...
def find_authors_nationalities():
    ...
    return "\n".join([f"{au.first_name} {au.last_name} is {au.nationality}" for au in authors])
...
def delete_review_by_id(_id: int):
    user = Review.objects.get(id=_id)
    result = f"Review by {user.reviewer_name} was deleted"
    user.delete()
    return result
...
def filter_authors_by_nationalities(nationality: str) -> str:
    ...
    return "\n".join([f"{au.biography}" if au.biography else f"{au.first_name} {au.last_name}" for au in authors])
...
def filter_authors_by_birth_year(first_y: int, second_y: int) -> str:
    authors = Author.objects.filter(birth_date__year__range=(first_y, second_y)).order_by("-birth_date")
    return "\n".join([f"{au.birth_date}: {au.first_name} {au.last_name}" for au in authors])
...
def change_reviewer_name(name: str, new_name: str):
    reviews = Review.objects.filter(reviewer_name=name)
    for rev in reviews:
        rev.reviewer_name = new_name
