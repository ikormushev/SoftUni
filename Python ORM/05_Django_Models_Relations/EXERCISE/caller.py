...
def show_all_authors_with_their_books() -> str:
    ...
    for au in authors:
        books = Book.objects.filter(author=au)

        if not books:
            continue

        result.append(f"{au.name} has written - {', '.join(b.title for b in books)}!")
...
def add_song_to_artist(artist_name: str, song_title: str):
    artist = Artist.objects.all().get(name=artist_name)
    song = Song.objects.all().get(title=song_title)

    artist.songs.add(song)


def get_songs_by_artist(artist_name: str):
    return Artist.objects.all().get(name=artist_name).songs.all().order_by("-id")


def remove_song_from_artist(artist_name: str, song_title: str):
    Artist.objects.all().get(name=artist_name).songs.remove(Song.objects.all().get(title=song_title))
...
def calculate_average_rating_for_product_by_name(product_name: str):
    ...
    reviews_averages = product.reviews.aggregate(average_rating=Avg("rating"))

    return reviews_averages["average_rating"]
...
def get_products_with_no_reviews():
    return Product.objects.all().filter(reviews__isnull=True).all().order_by("-name")


def delete_products_without_reviews():
    Product.objects.all().filter(reviews__isnull=True).all().delete()
...
def calculate_licenses_expiration_dates() -> str:
    ...
    return "\n".join([f"License with number: {dl.license_number} expires on {dl.issue_date + timedelta(365)}!"
                      for dl in licenses])


def get_drivers_with_expired_licenses(due_date: date):
    date_difference = due_date - timedelta(365)

    return Driver.objects.filter(license__issue_date__gt=date_difference)
...
def register_car_by_owner(owner: Owner) -> str:
    available_registration = Registration.objects.filter(car__isnull=True).first()
    car = Car.objects.filter(registration__isnull=True).first()

    car.owner = owner
    car.save()

    available_registration.registration_date = date.today()
    available_registration.car = car
    available_registration.save()

    return (f"Successfully registered {car.model} to {owner.name} with "
            f"registration number {available_registration.registration_number}.")
